...
//				console.log(n+' '+data.tableId+' '+data.outId+' '+data.value);
			if (typeof(data)!="undefined") {
		        ...
		        if (typeof(tableId) == "undefined"){
		        	thiz.loadAddBtn2(thiz.tableId,data.outId,data.value);
		        }
				thiz.loadAddBtn2(data.tableId,data.outId,data.value);
			}    
	    }); 
	 ...
	 * @author：xuwei
	 * “显示 xxx” 后面加载按钮
	 *  loadAddBtn (tableId,outId,value)
	 *  tableId:当前页面的table的id
	 *  outId：输出到前台页面的按钮的id
	 *  value:按钮的显示名称是什么
	 */
	loadAddBtn2 : function (tableId,outId,value){
		//在xxx_length中的xxx为PageDataTables的aoColumns的tableId
		jQuery('#'+tableId+'_length label')
		.after('<button style="margin-left: 20px;" class="publishbutton radius3" id='+outId+'>'+value+'</button>');		
	}
};
