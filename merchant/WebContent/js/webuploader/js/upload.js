(function($) {
	...
	$(function() {
		...
		uploader = WebUploader.create({
			...
			chunked : false,
			chunkSize : 512 * 1024,
			server : contextPath + '/system/upload/uploadPic.htm?fileType='
					+ encodeURI(encodeURI(jQuery('#fileType').val()))
					+ "&tableId=" + jQuery('#tableId').val() + "&tableName="
			...
			// runtimeOrder: 'flash',
			// server:
			// 'http://liaoxuezhi.fe.baidu.com/webupload/fileupload.php',
		...
		// http://192.168.1.54:80/merchant/upload/image/2015-12-25/4448246a-38dc-4608-826b-9b2dc4e09c56.png
		uploader.on('uploadSuccess', function(items, res) {
			var fileName = items.name;
			var fileUrl = res._raw;
...
var submit = function(callBack) {
	// var fileUrlsArr = fileUrls.split(',');
	// var fileNamesArr = fileNames.split(',');

	jQuery('#fileUrl').val(fileUrls);
	jQuery('#fileName').val(fileNames);

	// 保存图片
	jQuery.ajax({
		cache : true,
		type : "POST",
		url : contextPath + '/system/upload/savePic.htm?',
		...
		success : function(data) {
			if (data.msg.success) {
//				alert("主键：" + data.ids);
//				alert("URL：" + data.fileUrls);
				alert("提示：图片保存成功");
				callBack();

			} else {
				alert("提示：" + data.msg.info);
			}
		}
	});
