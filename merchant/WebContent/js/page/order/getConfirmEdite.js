jQuery(document).ready(function() {
	...
	jQuery("#schemingReturnTime").datetimepicker();
	...
	jQuery("#form1").validate({
		rules : {
			'kmsForGet' : {"required":true,"positiveNumber":true},
			'components' : "required",
			'statuss' : "required",
			'surplusKmsForGet':{"required":true,"positiveNumber":true},
			'describes' : "required",
...
var submit = function(callBack) {
	...
	if( schemingReturnTime+":00.0" < realityGetTime){
	...
	if (jQuery("#form1").valid()) {
		jQuery.ajax({
			cache : true,
			type : "POST",
			url : contextPath + '/order/getConfirm/saveEdite.htm?',
...
var uploadImg = function(thiz) {
	// var filepath = jQuery('#carImgFile').val();
	// var extStart = filepath.lastIndexOf(".");
	// var ext = filepath.substring(extStart, filepath.length).toUpperCase();
	// if (filepath != "") {
	// if (ext != ".PNG" && ext != ".JPG") {
