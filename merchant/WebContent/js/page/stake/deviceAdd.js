...
var chargePortCombox = new Combox(
		{
			id : 'chargePort',
			url : contextPath + '/stake/device/portTypes.htm?t='
					+ new Date().getTime(),
			cText : 'chargePort',
			cValue : 'chargePort',
			emptyText : '请选择充电端口类型',
			change : function() {
				if (jQuery('#chargePort').val() == "旗翔1托30") {
					jQuery('#hidNum').show();
				} else {
					jQuery('#hidNum').hide();
				}
...
var submit = function(callBack) {
	...
	if (jQuery("#form1").valid()) {
		jQuery.ajax({
			cache : true,
			type : "POST",
			url : contextPath + '/stake/device/saveAdd.htm?',
