jQuery(document).ready(function() {
	...
	jQuery("#form2").validate({
		rules : {
			'name' : "required",
			'PriceA' : {"required" : true,"number" : true},
			'PriceB' : {"required" : true,"number" : true},
			'PriceC' : {"required" : true,"number" : true},
			'PriceD' : {"required" : true,"number" : true},
		}
	});
...
var submit = function(callBackAdd) {
	if (jQuery("#time1").val() == ''||jQuery("#time2").val() == ''||jQuery("#time3").val() == ''||jQuery("#time4").val() == '') {
	...
	if (jQuery("#form2").valid()) {
		jQuery.ajax({
			cache : true,
			type : "POST",
			url : contextPath + '/stake/price/saveAdd.htm?',
