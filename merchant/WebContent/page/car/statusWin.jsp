...
<script type="text/javascript" src="${ctx}/js/page/car/statusWin.js"></script>
</head>
		...
		<div class="overviewselect">
			...
			<div class="overviewselect">
				<select id="curStatus" name="select" class="chzn-select"
					...
					<option value="空闲"
						<c:if test="${modelMap.status == '空闲' }">selected="selected"</c:if>>待租</option>
					<option value="使用"
						<c:if test="${modelMap.status == '使用' }">selected="selected"</c:if>>已租</option>
					<option value="充电"
						<c:if test="${modelMap.status == '充电' }">selected="selected"</c:if>>充电</option>
					<option value="故障"
