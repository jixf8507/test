...
<script type="text/javascript"
	src="${ctx}/js/page/stake/stakeCardEdite.js"></script>
</head>
<script type="text/javascript">
	var ComAxCtrl = null;
	function initss() {
...
<body>
	...
	<form id="form1" class="stdform stdform2" method="post" action="">
		<input type="hidden" name="id" id="id" value="${cardInfoPO.id}" />
			<p>
		...
		<p>
			<label>内置卡号</label> <span class="field"> <input type="text"
				style="width: 200px;" name="cardID" id="nzkh" value="${cardInfoPO.cardID}"
		...
		<%-- <p>
			<label>内置卡号</label> <span class="field"><input type="text"
				value="${cardInfoPO.cardID}" style="width: 242px;" name="cardID"
				id="cardID" class="longinput" />&nbsp;&nbsp;<input
								type="button" value="点击读取" onclick="getCardID()" /></span>
		</p> --%>
		<p>
			<label><c:if
					test="${sessionScope.user.systemName == 'stake'}">一卡通卡号</c:if> <c:if
					test="${sessionScope.user.systemName == 'eakay'}">外置卡号</c:if></label> <span
				class="field"> <input type="text"
				value="${cardInfoPO.cardNumber}" style="width: 200px;"
				name="cardNumber" id="cardNumber" class="longinput" />
			</span>
		</p>
		<p>
			<label>客户手机号码</label> <span class="field"> <input
				value="${cardInfoPO.customerPO.phone}" readonly="readonly"  name="phone"
				type="text" style="width: 200px;" id="phone" class="longinput" />&nbsp;&nbsp;<input
				type="button" value="点击获取" onclick="getCustomer()" />
			</span>
		</p>
		<p>
			<label>客户姓名</label> <span class="field"> <input type="text" 
				value="${cardInfoPO.customerPO.name}" style="width: 200px;"
				id="name" class="longinput" readonly="readonly" /> <input
				name="customerId" type="hidden" id="customerId"
				value="${cardInfoPO.customerPO.id}" />
			</span>
		</p>
		<p>
			<label>绑定车牌号码</label> <span class="field"> <input type="text"
				value="${cardInfoPO.carId}" style="width: 200px;" name="carId"
				id="carId" readonly="readonly" class="longinput" />&nbsp;&nbsp;<input
				type="button" value="获取商家车辆" onclick="openSelectCarWin()" />&nbsp;&nbsp;<input
				type="button" value="获取客户车辆" onclick="openSelectCustomerCarWin()" />
				<input type="hidden" name="carTableName" id="carTableName"
